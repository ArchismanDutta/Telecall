...
function CallerDashboard({ caller, calls, addCall, requestCall, getRemoteCall, updateRemoteCall, navigate, onLogout }) {
  ...
  useEffect(() => { if (!calling) return undefined; const timer = setInterval(() => setSeconds(s => s + 1), 1000); return () => clearInterval(timer) }, [calling])
...
function Login({ type, onLogin, onSwitch }) {
  ...
  const [error, setError] = useState('')
  const submit = e => { e.preventDefault(); if (!username || !password) { setError('Enter your username and password to continue.'); return } const result = onLogin(type, username.trim()); if (result === 'agent-portal') setError('This is an agent account. Use Agent sign in below.'); if (result === 'not-found') setError('No agent account matches that username.'); if (result === 'inactive') setError('This agent account is not active. Contact the administrator.') }
...
function App() {
  ...
  useEffect(() => { callers.forEach(agent => apiRequest('/agents/sync', { method: 'POST', body: JSON.stringify({ agentId: agent.id, username: agent.username, status: agent.status }) }).catch(() => {})) }, [callers])
  ...
  const login = (type, username) => { setRole(type); if (type === 'admin') { if (callers.some(c => c.username === username)) return 'agent-portal'; navigate('dashboard'); return true } const found = callers.find(c => c.username === username); if (!found) return 'not-found'; if (found.status !== 'Active') return 'inactive'; setCallerUser(found.id); navigate('caller'); return true }
